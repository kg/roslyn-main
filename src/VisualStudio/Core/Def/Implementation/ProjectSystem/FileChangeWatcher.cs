    ...
    internal sealed class FileChangeWatcher
    {
        ...
        private sealed class Context : IVsFreeThreadedFileChangeEvents, IContext
        {
            ...
            public void StopWatchingFile(IFileWatchingToken token)
            {
                ...
                if (typedToken == null)
                {
                    throw new ArgumentException("The token passed did not originate from this service.", nameof(token));
                }
                ...
                lock (_gate)
                {
                    if (!_activeFileWatchingTokens.Remove(typedToken))
                    {
                        throw new InvalidOperationException("This token was no longer being watched.");
                    }
                }
